...
const hoveredEntityId = ref<string | null>(null) // Hover no canva (apenas tooltip)
const listHoveredEntityId = ref<string | null>(null) // Hover na lista (ativa opacidade)
...
const zoomIn = () => { if(viewportScale.value < 2) viewportScale.value += 0.2 }
const zoomOut = () => { if(viewportScale.value > 0.5) viewportScale.value -= 0.2 }

</script>

<template>
  <div class="flex flex-col lg:flex-row h-screen bg-slate-50 text-slate-800 font-sans overflow-hidden">
    ...
    <main class="flex-1 flex flex-col bg-slate-100/50 overflow-hidden relative">
      
      <!-- Zoom Controls for Mobile -->
      <div class="fixed bottom-24 right-4 flex flex-col gap-2 z-40 lg:hidden">
         <button @click="zoomIn" class="w-10 h-10 bg-white border border-slate-200 shadow-lg rounded-sm flex items-center justify-center font-bold text-lg">+</button>
         <button @click="zoomOut" class="w-10 h-10 bg-white border border-slate-200 shadow-lg rounded-sm flex items-center justify-center font-bold text-lg">-</button>
      </div>

      <div class="flex-1 w-full overflow-auto flex items-center justify-center p-4 lg:p-12 hide-scrollbar scroll-smooth">
          <div class="relative origin-center transition-transform duration-200 ease-out"
               :style="{ transform: `scale(${viewportScale})` }">
               
               <div class="grid relative bg-white rounded-sm ring-1 ring-slate-200 select-none touch-none shadow-none"
                    :style="{ gridTemplateColumns: `repeat(${boardSize}, minmax(0, 1fr))`, gridTemplateRows: `repeat(${boardSize}, minmax(0, 1fr))`, width: '800px', height: '800px', flexShrink: 0 }">
                ...
                <div v-for="index in boardSize * boardSize" :key="`cell-${index}`"
                    ...
                    class="border-r border-b border-slate-50 bg-transparent relative box-border"
                ...
                <div v-if="dragX !== null && dragY !== null && dragType"
                    class="absolute pointer-events-none opacity-50 z-20"
                    :class="[entityDefs[dragType].class, isSpaceFree(dragX, dragY, entityDefs[dragType].w, entityDefs[dragType].h, dragMoveId || undefined) ? 'bg-emerald-200' : 'bg-rose-200']"
                    :style="{ 
                ...
                <div v-if="!hideEffectArea" class="absolute inset-0 w-full h-full pointer-events-none z-0">
                    <template v-for="ent in entities" :key="'effect-'+ent.id">
                        <div v-if="ent.type === 'flag'" class="absolute bg-blue-300/30 border border-blue-400/30"
                            :style="{ 
                ...
                <div v-for="ent in entities" :key="ent.id" 
                    class="absolute p-[1px] transition-none pointer-events-none"
                    :style="{ 
                    ...
                    <div draggable="true" 
                            ...
                            :class="[entityDefs[ent.type].class, 'w-full h-full flex items-center justify-center cursor-move overflow-hidden pointer-events-auto border-transparent', selectedEntityId === ent.id ? 'border-emerald-500 border-2 z-10' : '']">
                        <span v-if="entityDefs[ent.type].icon" :class="[entityDefs[ent.type].w >= 2 ? 'text-lg' : 'text-[10px]']">{{ entityDefs[ent.type].icon }}</span>
                        <span v-if="ent.cityNumber" class="font-bold text-slate-800/60 z-10" :class="[entityDefs[ent.type].w >= 2 ? 'text-sm ml-0.5' : 'text-[8px] ml-0.5']">{{ ent.cityNumber }}</span>
                    </div>

                    <!-- Tooltip above everything (Only for cities) -->
                    <div v-if="['rally', 'joiner', 'member'].includes(ent.type) && ent.name && (hoveredEntityId === ent.id || selectedEntityId === ent.id)" 
                         class="absolute -top-8 left-1/2 -translate-x-1/2 bg-slate-800 text-white text-[10px] px-2 py-1 rounded-sm whitespace-nowrap z-[100] shadow-sm pointer-events-none">
                      {{ ent.name }}
                    </div>
                </div>
              </div>
      ...
      <transition enter-active-class="transition duration-200 ease-out" enter-from-class="translate-y-10 opacity-0" enter-to-class="translate-y-0 opacity-100" leave-active-class="transition duration-150 ease-in" leave-from-class="translate-y-0 opacity-100" leave-to-class="translate-y-10 opacity-0">
        <div v-if="getSelectedEntity" class="fixed bottom-4 left-1/2 -track-x-1/2 w-[90%] lg:w-full lg:max-w-sm bg-white border border-slate-200 rounded-sm p-3 shadow-xl flex items-center gap-4 z-50 transform -translate-x-1/2">
           <div class="flex items-center gap-3 w-full">
             <div :class="entityDefs[getSelectedEntity.type].class" class="w-8 h-8 flex-shrink-0 relative flex items-center justify-center">
                 <span v-if="entityDefs[getSelectedEntity.type].icon" class="text-xs">{{ entityDefs[getSelectedEntity.type].icon }}</span>
                 <span v-if="getSelectedEntity.cityNumber" class="font-bold text-slate-800/60 text-[10px] ml-0.5">{{ getSelectedEntity.cityNumber }}</span>
             </div>
             <div class="flex-1">
               <input v-if="getSelectedEntity.name !== undefined" type="text" v-model="getSelectedEntity.name" class="w-full text-sm font-bold text-slate-800 bg-transparent border-b border-slate-100 focus:border-slate-500 focus:outline-none" placeholder="Name..." maxlength="16"/>
               <p v-else class="text-[10px] font-bold text-slate-500 uppercase">{{ entityDefs[getSelectedEntity.type].label }}</p>
             </div>
           </div>
           <button @click="entities = entities.filter(ent => ent.id !== getSelectedEntity!.id); selectedEntityId = null" class="ml-auto p-2 text-rose-500 bg-rose-50 rounded-sm">
    ...
    <aside :class="['w-full lg:w-64 bg-white border-l border-slate-200 flex flex-col overflow-y-auto lg:shrink-0 transition-all duration-300 z-20 shadow-none', 
      ...
      <div class="p-4 border-b border-slate-100 sticky top-0 bg-white/90 backdrop-blur z-20 flex justify-between items-center">
        <h3 class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Cities Active</h3>
      </div>
      <div class="flex flex-col gap-1 p-3 overflow-y-auto w-full">
        <div v-for="city in citiesList" :key="city.id" 
             ...
             :class="{'border-emerald-500 bg-emerald-50': selectedEntityId === city.id, 'border-slate-300 bg-slate-50': listHoveredEntityId === city.id && selectedEntityId !== city.id, 'border-slate-100 bg-white': selectedEntityId !== city.id && listHoveredEntityId !== city.id }">
          <div :class="entityDefs[city.type].colorClass" class="w-5 h-5 rounded-sm flex-shrink-0 flex items-center justify-center font-bold text-slate-800/60 text-[9px]">{{ city.cityNumber }}</div>
          <span class="text-xs font-medium text-slate-700 truncate flex-1">{{ city.name }}</span>
        </div>
        <div v-if="citiesList.length === 0" class="text-[10px] text-slate-400 text-center py-6 border-dashed border border-slate-200 rounded-sm italic">No cities deployed</div>
      </div>
    </aside>
  </div>
</template>
